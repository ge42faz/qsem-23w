...
\begin {equation}
C(\theta) = \qeval{H} = \qcovec{\psi(\theta)} H \qvec{\psi(\theta)}
\end {equation}

\section {Encoding Approaches}
A simple mapping from the VRP to QUBO uses what is known as the complete
encoding, where each classical binary variable is represented by a single
qubit.
